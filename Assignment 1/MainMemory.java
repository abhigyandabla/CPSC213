...
public class MainMemory extends AbstractMainMemory {
   ...
   */
  @Override public int bytesToInteger (byte byteAtAddrPlus0,
                                       byte byteAtAddrPlus1,
                                       byte byteAtAddrPlus2,
                                       byte byteAtAddrPlus3) {
      return (byteAtAddrPlus0 & 0xff)*16777216 +
             (byteAtAddrPlus1 & 0xff)*65536 +
             (byteAtAddrPlus2 & 0xff)*256 +
  ...
  @Override protected byte[] get (int address, int length) throws InvalidAddressException {
      if(address+length-1 > this.mem.length || address < 0 || length < 0){
  ...
  @Override protected void set (int address, byte[] value) throws InvalidAddressException {
      if(value.length+address-1 > this.mem.length || address < 0){
          throw new InvalidAddressException();
      }
